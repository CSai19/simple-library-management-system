...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        if (!empty($row["studentid"])) {
        	echo '<div class="sai">
                  <p class="para">The book was already taken<br>Select other book </p></div>';
        }
        else{
        	$query = "UPDATE Books SET studentid='$studentid',duedate=DATE_ADD(now(),INTERVAL 2 MONTH) WHERE id='$id'";
        	if (mysqli_query($conn, $query)) {
            echo '<div class="sai">
                  <p class="para">The book was added successfully to your account</p></div>';
			} else {
    		echo '<div class="sai">
                  <p class="para">Error adding book </p></div>';
			}
        }
    }
} else {
    echo '<div class="sai">
                  <p class="para">You have entered invalid of the book</p></div>';}
}
else
 	echo '<div class="sai">
                  <p class="para">You need to enter the id of the book </p></div>';
    
}
else
echo '<div class="sai">
                  <p class="para">You must be logged in first!!</p></div>';
